...
- [x] Map view — GPS is already captured per entry; a simple map screen pinning all entries would be a high-leverage addition for a "field data collection" app.
- [x] GPS component have the option street that automatically takes the street name and number by a gps button or location picker. — `captureLocation` (`utils/sensors.ts`) now reverse-geocodes after capture and stores a street address on `GpsLocation.address`; shown in the GPS field, entry detail map card, and edit-entry read-only view. Best-effort: a geocoder failure/timeout never blocks the capture itself, it just leaves the address blank.
