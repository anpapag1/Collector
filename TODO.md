...
- [ ] Edit existing entries — right now entries are create-once/read-only; field teams often need to correct a typo after the fact.
...
- [ ] Remote sync (Supabase) — store entries/photos remotely per-account, local-first with background sync (see plan).
